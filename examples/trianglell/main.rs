...
use gfx_corell::{buffer, command, format, pass, pso, shade, state, target, Device, CommandPool,
                 GraphicsCommandPool, CommandQueue, GraphicsCommandBuffer,
                 ProcessingCommandBuffer, TransferCommandBuffer, PrimaryCommandBuffer, Primitive,
                 Instance, Adapter, Surface, SwapChain, QueueFamily, QueueSubmit, Factory,
                 SubPass, FrameSync};
...
fn main() {
    ...
    let queue_descs = physical_devices[0]
        .get_queue_families()
    ...
    let Device {
        ...
        heap_types,
        ..
    } = physical_devices[0].open(queue_descs);
    let mut swap_chain = surface.build_swapchain::<ColorFormat>(&general_queues[0]);

    // Setup renderpass and pipeline
    #[cfg(all(target_os = "windows", not(feature = "vulkan")))]
    let shader_lib = factory
        .create_shader_library(&[("vs_main", include_bytes!("data/vs_main.o")),
                                 ("ps_main", include_bytes!("data/ps_main.o"))])
        .expect("Error on creating shader lib");

    #[cfg(feature = "vulkan")]
    let shader_lib = factory
        .create_shader_library(&[("vs_main", include_bytes!("data/vs_main.spv")),
    ...
    pipeline_desc.color_targets[0] =
        Some((ColorFormat::get_format(),
              state::Blend {
    ...
    let frame_rtvs = swap_chain
        .get_images()
        .iter()
        .map(|image| {
                 factory
                     .view_image_as_render_target(&image, ColorFormat::get_format())
                     .unwrap()
             })
        .collect::<Vec<_>>();

    let framebuffers = frame_rtvs
        .iter()
        .map(|frame_rtv| {
    ...
    let upload_heap = heap_types
        .iter()
        .find(|&&heap_type| {
                  heap_type
                      .properties
                      .contains(memory::CPU_VISIBLE | memory::COHERENT)
              })
        .unwrap();
    ...
    let vertex_buffer = {
        let buffer = factory
            .create_buffer(TRIANGLE.len() as u64 * std::mem::size_of::<Vertex>() as u64,
                           buffer::VERTEX)
            .unwrap();
        println!("{:?}", buffer);
    ...
    {
        let mut mapping = factory
            .write_mapping::<Vertex>(&vertex_buffer, 0, TRIANGLE.len() as u64)
            .unwrap();
    ...
    let image_upload_buffer = {
        let buffer = factory
            .create_buffer(img.len() as u64, buffer::TRANSFER_SRC)
            .unwrap();
        let buffer_req = factory.get_buffer_requirements(&buffer);
        factory
            .bind_buffer_memory(&image_upload_heap, 0, buffer)
            .unwrap()
    };
    ...
    // copy image data into staging buffer
    {
        ...
        let mut mapping = factory
            .write_mapping::<u8>(&image_upload_buffer, 0, img.len() as u64)
            .unwrap();
    ...
    let image_srv = factory
        .view_image_as_shader_resource(&image_logo, gfx_corell::format::Srgba8::get_format())
        .unwrap();

    let sampler =
        factory.create_sampler(i::SamplerInfo::new(i::FilterMethod::Bilinear, i::WrapMode::Clamp));

    factory.update_descriptor_sets(&[DescriptorSetWrite {
                                        set: &set0[0],
                                        binding: 0,
                                        array_offset: 0,
                                        write: DescriptorWrite::SampledImage(vec![(&image_srv,
    ...
    while running {
        events_loop.poll_events(|winit::Event::WindowEvent {
                                     window_id: _,
                                     event,
                                 }| {
            match event {
        ...
        let submit = {
...

            {
                let mut encoder = back::RenderPassInlineEncoder::begin(
                    ...
                    &[command::ClearValue::Color(command::ClearColor::Float([0.8, 0.8,
                        0.8, 1.0]))]);
