use super::native::{self, descriptor, pso};
use std::ffi::CStr;
use std::{mem, ptr};
...
impl BlitPipe {
    pub fn new(device: native::Device, (dst_format, filter): BlitKey) -> Self {
        ...
        let (signature, _hr) = device.create_root_signature(signature_raw, 0);
