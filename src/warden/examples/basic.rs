...
fn main() {
    ...
    let raw_scene = {
        ...
        File::open(base_path.join("scenes/basic.ron"))
            ...
            .unwrap();
        let mut deserializer = Deserializer::from_bytes(&raw_data).unwrap();
        warden::raw::Scene::deserialize(&mut deserializer).unwrap()
    };

    let events_loop = gl::glutin::EventLoop::new();
    let window = gl::glutin::GlWindow::new(
        gl::glutin::WindowBuilder::new(),
        gl::glutin::ContextBuilder::new().with_gl_profile(gl::glutin::GlProfile::Core),
        &events_loop,
    )
    .unwrap();
    ...
    let mut scene =
        warden::gpu::Scene::<gl::Backend, _>::new(adapter, &raw_scene, base_path.join("data"))
            .unwrap();
