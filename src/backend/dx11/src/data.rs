...
pub fn map_anti_alias(aa: AaMode) -> DXGI_SAMPLE_DESC {
    match aa {
        ...
        AaMode::Coverage(samples, fragments) => {
            DXGI_SAMPLE_DESC {
...
pub fn map_access(access: memory::Access) -> D3D11_CPU_ACCESS_FLAG {
    ...
    if access.contains(memory::READ) {
        r = r | D3D11_CPU_ACCESS_READ
    }
    if access.contains(memory::WRITE) {
        r = r | D3D11_CPU_ACCESS_WRITE
    }
    r
}
...
pub fn map_usage(usage: Usage) -> (D3D11_USAGE, D3D11_CPU_ACCESS_FLAG) {
    match usage {
        ...
        Usage::Mappable(access) => {
            if access.contains(memory::READ) {
                panic!("DX does not support mapped memory for read access!");
            } else {
