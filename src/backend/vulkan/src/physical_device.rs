...
use crate::{
    conv, info, xr, Backend, Device, DeviceExtensionFunctions, ExtensionFn, Queue, QueueFamily,
...
impl adapter::PhysicalDevice<Backend> for PhysicalDevice {
    unsafe fn open(
        ...
        if xr::in_use() {
            ...
            let xr_queue_family_index = families.first().map(|&(family, _)| family.index).unwrap();
