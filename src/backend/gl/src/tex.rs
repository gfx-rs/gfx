...
fn format_to_gltype(format: NewFormat) -> Result<GLenum, ()> {
    ...
    let (fm8, fm16, fm32) = match format.1 {
        C::Int | C::Inorm => (gl::BYTE, gl::SHORT, gl::INT),
        C::Uint | C::Unorm | C::Srgb => (gl::UNSIGNED_BYTE, gl::UNSIGNED_SHORT, gl::UNSIGNED_INT),
    ...
    Ok(match format.0 {
        ...
        S::R16_G16 |
        S::R16_G16_B16 |
        S::R16_G16_B16_A16 => fm16,
        S::R32 |
        S::R32_G32 |
        S::R32_G32_B32 |
        S::R32_G32_B32_A32 => fm32,
...
fn make_surface_impl(gl: &gl::Gl, format: GLenum, dim: t::Dimensions) -> Result<Surface, ()> {
    ...
    match dim.3 {
        t::AaMode::Single => unsafe {
            gl.RenderbufferStorage(target, format, dim.0 as GLsizei, dim.1 as GLsizei);
        },
...
pub fn make_surface(gl: &gl::Gl,
    ...
    make_surface_impl(gl, fmt, desc.kind.get_dimensions()).map_err(|_| format_error)
}

fn make_widout_storage_impl(gl: &gl::Gl,
                            kind: t::Kind,
                            format: GLint,
                            pix: GLenum,
                            typ: GLenum,
    ...
    match kind {
        ...
        t::Kind::D2Array(w, h, a, t::AaMode::Multi(samples)) => unsafe {
            gl.TexImage3DMultisample(target,
                                     samples as GLsizei,
                                     format as GLenum, // GL spec bug
                                     w as GLsizei,
                                     h as GLsizei,
        ...
        t::Kind::Cube(w) => {
            for &target in [gl::TEXTURE_CUBE_MAP_POSITIVE_X,
...
pub fn make_without_storage(gl: &gl::Gl,
                            desc: &t::Info,
                            cty: ChannelType)
                            -> Result<Texture, t::CreationError> {
    let format = NewFormat(desc.format, cty);
...
fn make_with_storage_impl(gl: &gl::Gl,
                          kind: t::Kind,
                          format: GLenum,
                          levels: t::Level,
                          fixed_sample_locations: bool)
    ...
    match kind {
        t::Kind::D1(w) => unsafe {
            gl.TexStorage1D(target, min(levels, mip_level1(w)), format, w as GLsizei);
        },
        t::Kind::D1Array(w, a) => unsafe {
        ...
        t::Kind::D2(w, h, t::AaMode::Multi(samples)) => unsafe {
            gl.TexStorage2DMultisample(target,
                                       samples as GLsizei,
                                       format,
                                       w as GLsizei,
                                       h as GLsizei,
        ...
        t::Kind::D2Array(w, h, a, t::AaMode::Multi(samples)) => unsafe {
            gl.TexStorage3DMultisample(target,
                                       samples as GLsizei,
                                       format as GLenum,
                                       w as GLsizei,
                                       h as GLsizei,
...
pub fn make_with_storage(gl: &gl::Gl,
                         desc: &t::Info,
                         cty: ChannelType)
                         -> Result<Texture, t::CreationError> {
    let format = NewFormat(desc.format, cty);
...
pub fn bind_sampler(gl: &gl::Gl, target: GLenum, info: &t::SamplerInfo, is_embedded: bool) {
    unsafe {
...
fn update_texture_impl<F>(gl: &gl::Gl,
                          kind: t::Kind,
                          target: GLenum,
                          pix: GLenum,
                          typ: GLenum,
    ...
    Ok(match kind {
        ...
        t::Kind::D1Array(_, _) |
        t::Kind::D2(_, _, t::AaMode::Single) => unsafe {
...
pub fn update_texture(gl: &gl::Gl,
                      name: Texture,
                      kind: t::Kind,
...
pub fn init_texture_data(gl: &gl::Gl,
                         name: Texture,
                         desc: t::Info,
                         channel: ChannelType,
                         data: &[&[u8]])
...
pub fn make_sampler(gl: &gl::Gl, info: &t::SamplerInfo) -> Sampler {
    unsafe {
...
pub fn generate_mipmap(gl: &gl::Gl, name: Texture, target: gl::types::GLenum) {
    unsafe {
