...
fn get_block_iv(gl: &gl::Gl,
                name: super::Program,
                index: gl::types::GLuint,
                query: gl::types::GLenum)
                -> gl::types::GLint {
    let mut iv = 0;
...
impl StorageType {
    fn new(storage: gl::types::GLenum) -> StorageType {
        ...
        match storage {
            ...
            gl::FLOAT_MAT2 => {
                Var(BaseType::F32,
                    ContainerType::Matrix(MatrixFormat::ColumnMajor, 2, 2))
            }
            gl::FLOAT_MAT3 => {
                Var(BaseType::F32,
                    ContainerType::Matrix(MatrixFormat::ColumnMajor, 3, 3))
            }
            gl::FLOAT_MAT4 => {
                Var(BaseType::F32,
                    ContainerType::Matrix(MatrixFormat::ColumnMajor, 4, 4))
            }
            gl::FLOAT_MAT2x3 => {
                Var(BaseType::F32,
                    ContainerType::Matrix(MatrixFormat::ColumnMajor, 2, 3))
            }
            gl::FLOAT_MAT2x4 => {
                Var(BaseType::F32,
                    ContainerType::Matrix(MatrixFormat::ColumnMajor, 2, 4))
            }
            gl::FLOAT_MAT3x2 => {
                Var(BaseType::F32,
                    ContainerType::Matrix(MatrixFormat::ColumnMajor, 3, 2))
            }
            gl::FLOAT_MAT3x4 => {
                Var(BaseType::F32,
                    ContainerType::Matrix(MatrixFormat::ColumnMajor, 3, 4))
            }
            gl::FLOAT_MAT4x2 => {
                Var(BaseType::F32,
                    ContainerType::Matrix(MatrixFormat::ColumnMajor, 4, 2))
            }
            gl::FLOAT_MAT4x3 => {
                Var(BaseType::F32,
                    ContainerType::Matrix(MatrixFormat::ColumnMajor, 4, 3))
            }

            // TODO: double matrices
            gl::SAMPLER_1D => {
                Sampler(BaseType::F32,
                        TextureType::D1(NoArray),
                        SamplerType(NoCompare, NoRect))
            }
            gl::SAMPLER_1D_ARRAY => {
                Sampler(BaseType::F32,
                        TextureType::D1(Array),
                        SamplerType(NoCompare, NoRect))
            }
            gl::SAMPLER_1D_SHADOW => {
                Sampler(BaseType::F32,
                        TextureType::D1(NoArray),
                        SamplerType(Compare, NoRect))
            }
            ...
            gl::SAMPLER_2D => {
                Sampler(BaseType::F32,
                        TextureType::D2(NoArray, NoMultiSample),
                        SamplerType(NoCompare, NoRect))
            }
            gl::SAMPLER_2D_ARRAY => {
                Sampler(BaseType::F32,
                        TextureType::D2(Array, NoMultiSample),
                        SamplerType(NoCompare, NoRect))
            }
            ...
            gl::SAMPLER_2D_MULTISAMPLE => {
                Sampler(BaseType::F32,
                        TextureType::D2(NoArray, MultiSample),
                        SamplerType(NoCompare, NoRect))
            }
            gl::SAMPLER_2D_RECT => {
                Sampler(BaseType::F32,
                        TextureType::D2(NoArray, NoMultiSample),
                        SamplerType(NoCompare, Rect))
            }
            ...
            gl::SAMPLER_2D_MULTISAMPLE_ARRAY => {
                Sampler(BaseType::F32,
                        TextureType::D2(Array, MultiSample),
                        SamplerType(NoCompare, NoRect))
            }
            gl::SAMPLER_2D_RECT_SHADOW => {
                Sampler(BaseType::F32,
                        TextureType::D2(NoArray, NoMultiSample),
                        SamplerType(Compare, Rect))
            }
            gl::SAMPLER_3D => {
                Sampler(BaseType::F32,
                        TextureType::D3,
            ...
            gl::INT_SAMPLER_BUFFER => {
                Sampler(BaseType::I32,
                        TextureType::Buffer,
                        SamplerType(NoCompare, NoRect))
            }

            gl::UNSIGNED_INT_SAMPLER_1D => {
                Sampler(BaseType::U32,
                        TextureType::D1(NoArray),
                        SamplerType(NoCompare, NoRect))
            }
...
fn query_blocks(gl: &gl::Gl,
                caps: &c::Capabilities,
                prog: super::Program,
                block_indices: &[gl::types::GLint],
                block_offsets: &[gl::types::GLint])
    ...
    let bindings: Vec<gl::types::GLuint> = (0..num)
        .map(|idx| {
            get_block_iv(gl,
                         prog,
                         idx as gl::types::GLuint,
                         gl::UNIFORM_BLOCK_BINDING) as gl::types::GLuint
        })
        .collect();
    ...
    (0..num as gl::types::GLuint)
        ...
        .map(|(idx, &bind)| {
            ...
            let name = unsafe {
                ...
                let mut real_size = 0;
                gl.GetActiveUniformBlockName(prog,
                                             idx,
                                             size,
                                             &mut real_size,
            ...
            let usage = {
                let usage_list = [(s::VERTEX, gl::UNIFORM_BLOCK_REFERENCED_BY_VERTEX_SHADER),
            ...
            let slot = if explicit_binding {
                ...
                unsafe {
                    gl.UniformBlockBinding(prog, idx, idx);
                }
                idx
            };
            ...
            s::ConstantBufferVar {
                ...
                usage: usage,
                elements: block_indices.iter()
                    .zip(block_offsets.iter())
                    .enumerate()
                    .filter_map(|(i, (parent, offset))| {
...
fn query_parameters(gl: &gl::Gl,
    ...
    if caps.constant_buffer_supported {
        unsafe {
            gl.GetActiveUniformsiv(prog,
                                   total_num as gl::types::GLsizei,
                                   (&indices[..]).as_ptr(),
...
fn query_outputs(gl: &gl::Gl, prog: super::Program) -> (Vec<s::OutputVar>, bool) {
    ...
    for i in 0..num_slots as u32 {
        ...
        unsafe {
            gl.GetProgramResourceName(prog,
                                      gl::PROGRAM_OUTPUT,
                                      i,
                                      length,
                                      ptr::null_mut(),
...
pub fn create_program(gl: &gl::Gl,
                      caps: &c::Capabilities,
                      private: &PrivateCaps,
...
pub fn bind_uniform(gl: &gl::Gl, loc: gl::types::GLint, uniform: s::UniformValue) {
    ...
    match uniform {
        ...
        UniformValue::F32Matrix2(val) => unsafe {
            gl.UniformMatrix2fv(loc, 1, gl::FALSE, val[0].as_ptr())
        },
        UniformValue::F32Matrix3(val) => unsafe {
            gl.UniformMatrix3fv(loc, 1, gl::FALSE, val[0].as_ptr())
        },
        UniformValue::F32Matrix4(val) => unsafe {
            gl.UniformMatrix4fv(loc, 1, gl::FALSE, val[0].as_ptr())
        },
    }
}
