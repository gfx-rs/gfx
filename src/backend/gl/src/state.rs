...
use core::state as s;
use core::state::{BlendValue, Comparison, CullFace, Equation, Offset, RasterMethod, StencilOp,
                  FrontFace};
...
pub fn bind_raster_method(gl: &gl::Gl, method: s::RasterMethod, offset: Option<s::Offset>) {
    ...
    match offset {
        Some(Offset(factor, units)) => unsafe {
            gl.Enable(gl_offset);
            gl.PolygonOffset(factor as gl::types::GLfloat, units as gl::types::GLfloat);
        },
...
pub fn bind_viewport(gl: &gl::Gl, rect: Rect) {
    unsafe {
        gl.Viewport(rect.x as gl::types::GLint,
...
pub fn bind_scissor(gl: &gl::Gl, rect: Option<Rect>) {
    match rect {
        Some(r) => unsafe {
            ...
            gl.Scissor(r.x as gl::types::GLint,
...
pub fn bind_stencil(gl: &gl::Gl,
                    stencil: &Option<s::Stencil>,
                    refs: (Stencil, Stencil),
                    cull: s::CullFace) {
    fn bind_side(gl: &gl::Gl, face: gl::types::GLenum, side: s::StencilSide, ref_value: Stencil) {
        unsafe {
            gl.StencilFuncSeparate(face,
                                   map_comparison(side.fun),
                                   ref_value as gl::types::GLint,
                                   side.mask_read as gl::types::GLuint);
...
pub fn bind_blend(gl: &gl::Gl, color: s::Color) {
    match color.blend {
        Some(b) => unsafe {
            gl.Enable(gl::BLEND);
            gl.BlendEquationSeparate(map_equation(b.color.equation),
                                     map_equation(b.alpha.equation));
            gl.BlendFuncSeparate(map_factor(b.color.source),
    ...
    unsafe {
        gl.ColorMask(if (color.mask & s::RED).is_empty() {
                         gl::FALSE
                     } else {
                         gl::TRUE
                     },
                     if (color.mask & s::GREEN).is_empty() {
                         gl::FALSE
                     } else {
                         gl::TRUE
                     },
                     if (color.mask & s::BLUE).is_empty() {
                         gl::FALSE
                     } else {
                         gl::TRUE
                     },
                     if (color.mask & s::ALPHA).is_empty() {
                         gl::FALSE
                     } else {
                         gl::TRUE
                     })
