...
pub fn get(gl: &gl::Gl) -> (Info, Capabilities, PrivateCaps) {
    ...
    let tessellation_supported =
        info.is_version_or_extension_supported(4, 0, "GL_ARB_tessellation_shader");
    let caps = Capabilities {
        ...
        max_patch_size: if tessellation_supported {
            get_usize(gl, gl::MAX_PATCH_VERTICES)
        } else {
            0
        },
        ...
        srgb_color_supported:
            info.is_version_or_extension_supported(3, 2, "GL_ARB_framebuffer_sRGB"),
        constant_buffer_supported:
            info.is_version_or_extension_supported(3, 1, "GL_ARB_uniform_buffer_object"),
        unordered_access_view_supported: info.is_version_supported(4, 0), // TODO: extension
        separate_blending_slots_supported:
            info.is_version_or_extension_supported(4, 0, "GL_ARB_draw_buffers_blend"),
        copy_buffer_supported: info.is_version_or_extension_supported(3, 1, "GL_ARB_copy_buffer") |
                               ...
                               (info.is_embedded_version_supported(2, 0) &
                                info.is_extension_supported("GL_NV_copy_buffer")),
    };
    let private = PrivateCaps {
        ...
        program_interface_supported:
            info.is_version_or_extension_supported(4, 3, "GL_ARB_program_interface_query"),
        buffer_storage_supported:
            info.is_version_or_extension_supported(4, 4, "GL_ARB_buffer_storage"),
        clear_buffer_supported: info.is_version_supported(3, 0) |
                                info.is_embedded_version_supported(3, 0),
    };
    (info, caps, private)
}
