...
impl f::Factory<R> for Factory {
    ...
    fn create_texture_raw(&mut self, desc: t::Info, hint: Option<ChannelType>, mipmap: Option<t::Mipmap>, data_opt: Option<&[&[u8]]>)
        ...
        let object = if desc.bind.intersects(SHADER_RESOURCE | UNORDERED_ACCESS) || data_opt.is_some() {
            let name = if caps.immutable_storage_supported {
                try!(tex::make_with_storage(gl, &desc, cty, mipmap))
            } else {
                try!(tex::make_without_storage(gl, &desc, cty, mipmap))
            };
